...
sequenceDiagram
    participant User
    participant Server as Servidor do Agente<br/>(HTTP :8088)
    ...
    Agent-->>Server: Resposta do Resumo Executivo
...
sequenceDiagram
    participant User
    participant Agent as Agente de Resumo Executivo
    participant Model as Modelo Azure AI
    participant Tool as Função de Ferramenta Python

    User->>Agent: Mensagem do usuário
